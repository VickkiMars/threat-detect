...
| **T3.2** | B5.3, B6.2 | Verify real-time threat alert lifecycle: validate severity thresholding (LOW, MEDIUM, HIGH, CRITICAL), instant operator acknowledgment via dashboard REST API, and alert queue state synchronization. | S | T3.1 | In Progress |
| **T3.3** | DoD | Expand automated test suite coverage: implement dedicated test modules for balancing (`test_balancing.py`), model architectures (`test_models.py`), quantization (`test_quantization.py`), and evaluation (`test_evaluation.py`). | M | T3.1 | Ready |
